...
CREATE TABLE DinhMucNguyenLieu (
    ...
    maMon     VARCHAR(20)   NULL,       -- NULL nếu là định mức cho topping
    maTopping VARCHAR(20)   NULL,       -- NULL nếu là định mức cho món
    maNL      VARCHAR(20)   NOT NULL,

    CONSTRAINT PK_DinhMuc              PRIMARY KEY (maDinhMuc),
    CONSTRAINT FK_DinhMuc_Mon          FOREIGN KEY (maMon)     REFERENCES Mon(maMon),
    CONSTRAINT FK_DinhMuc_Topping      FOREIGN KEY (maTopping) REFERENCES Topping(maTopping),
    CONSTRAINT FK_DinhMuc_NL           FOREIGN KEY (maNL)      REFERENCES NguyenLieu(maNL),
    CONSTRAINT CHK_DinhMuc_SL          CHECK (soLuong > 0),
    CONSTRAINT CHK_DinhMuc_MonOrTop    CHECK (
        (maMon IS NOT NULL AND maTopping IS NULL) OR
